...
export const fetchMovies =createAsyncThunk("netflix/trending",async({type},thunkApi)=>{
    ...
    return getRawData(`${TMBD_BASE_URL}/trending/${type}/week?api_key=${API_KEY}`,genres,true);//Here we are getting the trending list of movies according to the given api list
...
// return getRawData(`${TMBD_BASE_URL}.discover/${type}?api_key=${API_KEY}&wih_genres=${}`)
...
const createArrayFromRawData=(array,moviesArray,genres)=>{ 
    ...
    array.forEach((movie)=>{
        ...
        if(movie.backdrop_path){//If has a poster then only we want this movie
            moviesArray.push({
                id:movie.id,
                name:movie?.original_name ? movie.original_name : movie.original_title,
                image:movie.backdrop_path,//poster of movie
...
const NetflixSlice = createSlice({
    ...
    extraReducers:(builder)=>{
        ...
        builder.addCase(fetchMovies.fulfilled,(state,action)=>{
            state.movies=action.payload;//When fetchMOvies is fulfilled then state will return payload
             
        });
        builder.addCase(fetchDataByGenre.fulfilled,(state,action)=>{
            state.movies=action.payload;//When fetchdataByGenre is fulfilled then state will return payload
